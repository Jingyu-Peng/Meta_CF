...
class model3_linear(torch.nn.Module):
    ...
    #            print(user_embedding)
                ...
                grad1_b = torch.autograd.grad(user_embedding.sum(), self.generate_layer.bias)[0]
                grad1_b = torch.tensor([list(grad1_b)]).cuda()
                ...
                grad2_b = tmp_grad.repeat(32, 1)
                grad_w = torch.mm(grad2_w.t(), grad1_w).cuda()
                grad_b = torch.mm(grad2_b.t(), grad1_b.t()).cuda()
                grad_w_sum = grad_w_sum.cuda() + grad_w
                grad_b_sum = grad_b_sum.cuda() + grad_b
        ...
        tmp_state_dict['generate_layer.bias'] = tmp_state_dict['generate_layer.bias'] - self.global_lr * grad_b.view(-1)
